...
void Linear::updateWeights(float learning_rate) {
    ...
    if (output_dim > 500) {
        effective_lr *= 8.0f; // Aumentar multiplicador para acelerar aprendizaje más agresivamente
        std::cout << "[LINEAR] Output projection - using 8x learning rate: " << effective_lr << std::endl;
    }
